## Task: upgrade a WhatsApp ordering bot from a rigid intent-classifier to a genuinely context-aware conversational agent

I run "Tandoori Junction," a real restaurant, with a live WhatsApp ordering bot (Flask app, deployed on Render, WhatsApp Cloud API for messaging, Groq for LLM calls). The infrastructure side (WhatsApp Business number, access tokens, webhook, hosting) is fully working — do not touch anything related to that. This is purely an application-logic upgrade.

### The problem

The bot currently *looks* like it understands natural language, but it doesn't really. It's a finite-state machine (stage: new → welcome → menu → subcategory → confirming) with an LLM call bolted on for intent classification. Each incoming message is classified into one of a fixed list of intents (greeting, menu, category, order, clear_cart, cart, confirm, cancel, faq, back, unknown), and each intent maps to a hardcoded template reply string. Concretely, this breaks down as soon as the customer says anything slightly off-script:

- The LLM classifier gets **no conversation history** — only the single current message plus a few state flags (current stage name, current category name, a flattened cart summary string). It has no memory of what was said 1–2 turns ago.
- If the message doesn't match one of the enumerated intents, and the bot's internal `stage` isn't literally the string `"confirming"`, the fallback is a hardcoded "Samajh nahi aaya! MENU likhein..." ("Didn't understand, type MENU...") — even when a human would trivially understand what was meant from context. Example we hit in testing: mid-conversation, replying "ok" got the generic "didn't understand" message instead of being read as an acknowledgment/continuation.
- Replies are canned template strings (`GREETING_TEXT`, `CATEGORY_MENU`, `FAQ_TEXT`, `"Kuch samajh nahi aaya..."`, etc.), not generated by the LLM — so the bot can't phrase a natural, context-appropriate response; it can only pick from a small set of pre-written strings.
- There's a `session["history"]` field initialized in `new_session()` that is **never actually populated or used anywhere** — it's dead code. The intent was presumably to give the LLM conversational memory, but that never got wired up.
- There are two separate, inconsistent brains: the Groq LLM path (`parse_message_with_llm`), and a much dumber keyword-matching fallback (`detect_intent` / `legacy_intent_reply`) used only when the Groq call errors out — so reliability of "understanding" silently degrades under load/failure with no visibility to the customer.

### What "good" looks like

I want the bot to feel like talking to a competent human staff member who's tracking the whole conversation, not a decision tree. Concretely:

- It should correctly handle ambiguous, short, colloquial, or context-dependent replies ("ok", "haan", "wahi wala", "nahi woh wala hata do", "ek aur", "bas itna hi", corrections like "chicken nahi paneer bolo tha", mixed Hindi/English/typos) by reasoning over what was actually said and done earlier in the conversation, not just the current message in isolation.
- It should be able to recover gracefully from genuinely unclear input by asking a smart, specific clarifying question grounded in context (not a generic "didn't understand, type MENU").
- Replies should be generated/composed with real language understanding rather than always being a fixed template string, while staying in the bot's established tone (Hinglish, casual, "Riya" persona, short WhatsApp-style messages).

### Hard constraints — do not compromise these while fixing the above

This is a live production bot taking real paying customers' food orders, so:

1. **Menu items and prices must never be hallucinated or altered.** Every item name and price the bot ever states or adds to a cart must come from the `CATEGORIES` / `ITEM_LOOKUP` data structures in the code — never invented or approximated by the LLM. Whatever architecture you use, keep a hard validation layer between "what the LLM thinks the customer wants" and "what actually gets added to the cart and billed."
2. **Order totals must be computed deterministically in code** (sum of real item prices × quantities), never stated as free text by the LLM.
3. Keep the existing surrounding systems working as-is: Flask + gunicorn on Render, WhatsApp Cloud API `send_meta_message` sending flow, SQLite `orders` table + schema/migrations, the `/dashboard` order-tracking UI, the owner-alert-with-retry system (`resend_pending_alerts`, delivery-status webhook handling), the `/privacy` route, and message-dedup via `_already_processed`. None of that is broken — don't rewrite it unless it's directly in the way of the fix.
4. Sessions are currently an in-memory `dict` keyed by phone number (`sessions[phone]`), reset on process restart. That's an existing known limitation (free Render tier restarts); fine to leave as-is unless there's an easy improvement, but don't make conversational memory depend on anything more fragile than that.
5. It's a free/cheap-tier setup (Groq `openai/gpt-oss-20b`, Render free web service) — be mindful of cost/latency (WhatsApp webhook needs to respond reasonably fast; Render free tier cold-starts already add delay).

### What I'd like from you

Redesign the conversational layer so the LLM actually reasons over the real conversation (proper message history, not just a stage label), and ideally move from "classify intent → pick template reply" toward something closer to an agentic pattern: the LLM sees full context and decides what to do (show category, add item(s), remove item(s), show cart, ask for clarification, confirm order, etc.) — likely via tool/function calls that map to the existing deterministic cart/menu operations — and composes the actual reply text itself, grounded strictly in real menu/cart data per the constraints above.

Feel free to restructure `parse_message_with_llm`, the intent enum/schema, `sessions`/`new_session`, and the reply-generation logic as needed. Keep everything else in the file intact. Below is the complete current `app.py` for full context.
